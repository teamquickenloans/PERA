...
function bzmDatePicker(a, b, c) {
    function d(a, c, d, f) {
        f.$formatters.unshift(function (b) {
            ...
            switch (a.viewMode) {
                case 0:
                    a.find(".prev").css(c <= a.startDate.getFullYear() && d <= a.startDate.getMonth() ? {
                        visibility: "hidden"
                    } : {
                        visibility: "visible"
                    }), a.find(".next").css(c >= a.endDate.getFullYear() && d >= a.endDate.getMonth() ? {
                        visibility: "hidden"
                    } : {
                        visibility: "visible"
                    });
                ...
                case 1:
                case 2:
                    a.find(".prev").css(c <= a.startDate.getFullYear() ? {
                        visibility: "hidden"
                    } : {
                        visibility: "visible"
                    }), a.find(".next").css(c >= a.endDate.getFullYear() ? {
                        visibility: "hidden"
                    } : {
                        visibility: "visible"
                    })
            ...
            switch (e.tagName) {
                case "TH":
                    if (e.classList.contains("date-switch") && a.showMode(1), e.classList.contains("prev") || e.classList.contains("next")) {
                        var f = h.modes[a.viewMode].navStep * (e.classList.contains("prev") ? -1 : 1);
                        switch (a.viewMode) {
                            case 0:
                                a.viewDate = a.moveMonth(a.viewDate, f);
                                break;
                            case 1:
                            case 2:
                                a.viewDate = a.moveYear(a.viewDate, f)
                        }
                        a.fill()
                    } else if (e.classList.contains("today")) {
                ...
                case "SPAN":
                    if (!e.classList.contains("disabled")) {
                        if (e.classList.contains("month")) {
                            for (var g = d.parent().find("span"), i = 0; i < g.length; i++)
                                if (d.text() === g.eq(i).text()) {
                                    a.viewDate.setMonth(i);
                                    break
                                }
                        } else {
                            var j = parseInt(d.text(), 10) || 0;
                            a.viewDate.setFullYear(j)
                        }
                        a.showMode(-1), a.fill()
                    }
                    break;
                case "TD":
                    if (e.classList.contains("day") && !e.classList.contains("disabled")) {
                        var k = parseInt(d.text(), 10) || 1,
                            j = a.viewDate.getFullYear(),
                            l = a.viewDate.getMonth();
                        d.hasClass("old") ? 0 === l ? (l = 11, j -= 1) : l -= 1 : d.hasClass("new") && (11 === l ? (l = 0, j += 1) : l += 1), a.setDate(new Date(j, l, k, 0, 0, 0, 0))
                    }
            }
            ...
            a.startDate = b ? b : new Date(0, 0, 0)
        }, a.setEndDate = function (b) {
            a.endDate = b ? b : new Date(3e3, 0, 0)
        }, a.show = function (c) {
            a.ngModel || (a.ngModel = new Date), a.update(), a.place(), a.viewMode = 0, a.showPicker = !0, b.on("keypress", a.keydown), c && a.$apply()
        }, a.hide = function (c) {
            a.showPicker = !1, a.picker.off("mousedown"), b.unbind("keypress", a.keydown), c && a.$apply()
        }, a.displayPicker = function () {
            a.picker && (a.showPicker ? a.hide() : (a.bindevent(a.picker), a.show()))
        }, a.bindevent = function (b) {
            function c(b) {
                b.preventDefault(), b.explicitOriginalTarget ? a.onclick(b.explicitOriginalTarget) : b.toElement ? a.onclick(b.toElement) : b.target ? a.onclick(b.target) : alert("Browser not supported [report a bug]", b)
            }
            b.on("mousedown", c)
        }, a.init = function () {
            if (a.dayoff = [], d.dayoff)
                for (var b = d.dayoff.split(","), c = 0; c < b.length; c++) a.dayoff.push(parseInt(b[c]));
            a.pickerid = d.id || "date-picker+" + parseInt(1e3 * Math.random()), a.language = d.language || a.locale || "en", a.autohide = d.autohide || !0, a.weekStart = d.weekstart || 1, a.calendarWeeks = d.weeknum || !1, a.todayButton = d.today || !1, a.todayHighlight = d.highlight || !0, a.placeholder = d.placeholder || locales.placeholder, a.format = d.format || a.datefmt || "dd-MM-yyyy", a.locales = e[a.language], a.setStartDate(a.notBefore), a.setEndDate(a.notAfter), d.today && "true" === a.todayButton.toLowerCase() && (a.todayButton = a.locales.today), a.$watch("notBefore", function () {
    ...
    var e = {
        ...
        g = '<div class="bzm-date-picker"> <div ng-click="displayPicker()" class="date-display"> <input class="date-input" placeholder="{{placeholder}}" value="{{modelviewvalue}}"></div><div ng-show="showPicker" class="datepicker datepicker-dropdown"><div ng-show="viewMode === 0" class="datepicker-days"><table class=" table-condensed">' + f.headTemplateDays + "<tbody></tbody>" + f.footTemplateDays + '</table></div><div ng-show="viewMode === 1" class="datepicker-months"><table class="table-condensed">' + f.headTemplate + f.contTemplate + f.footTemplate + '</table></div><div ng-show="viewMode === 2" class="datepicker-years"><table class="table-condensed">' + f.headTemplate + f.contTemplate + f.footTemplate + '</table></div><a class="button datepicker-close small alert right" style="width:auto;"><i class="fa fa-remove fa-times fi-x"></i></a></div>',
        h = {
            modes: [{
                clsName: "days",
                navFnc: "Month",
